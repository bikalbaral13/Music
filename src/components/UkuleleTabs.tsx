// Ukulele tablature view. Standard (re-entrant) GCEA tuning:
// string 1 (top in tab) = A4, string 2 = E4, string 3 = C4, string 4 = G4 (high G).
...
// Index 0 = lowest-pitched note as drawn at the bottom of the diagram.
// In re-entrant GCEA tuning C4 (60) is the lowest sounding string,
// then E4, G4, A4. We render C at the bottom for tab readability.
const STRINGS = [
  { name: 'C', openMidi: 60 },
  { name: 'E', openMidi: 64 },
  { name: 'G', openMidi: 67 },
...
function pickStringFret(midi: number, capo: number):
  ...
  // Prefer the highest-pitched string that can play this note within range —
  // matches how a player picks the closest position on the neck.
...
export default function UkuleleTabs({ activeMidi, showLabels, capo = 0 }: Props) {
  ...
  return (
    <div className="rounded-md bg-rose-50 border border-rose-200 p-3">
      <div className="flex items-baseline justify-between mb-1">
        <span className="text-xs font-medium text-slate-600">Ukulele tablature (GCEA, re-entrant)</span>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" role="img" aria-label="Ukulele tab">
        <rect x={padL} y={padT} width={fretboardW} height={fretboardH}
              fill="#fff1e6" stroke="#9a5a3b" strokeWidth={1}/>
        ...
        {[5, 7, 10, 12].map((f) => {
          ...
          if (f === 12) {
            return (
              <g key={f}>
                <circle cx={x} cy={cy - stringGap / 2} r={3.5} fill="#9a5a3b" opacity={0.6}/>
                <circle cx={x} cy={cy + stringGap / 2} r={3.5} fill="#9a5a3b" opacity={0.6}/>
              </g>
            );
          }
          return <circle key={f} cx={x} cy={cy} r={3.5} fill="#9a5a3b" opacity={0.6}/>;
        })}

        {Array.from({ length: FRETS }).map((_, i) => {
          const x = padL + (i + 0.5) * fretW;
          return <text key={i} x={x} y={padT + fretboardH + 18}
                       fontSize={11} textAnchor="middle" fill="#9a5a3b">{i + 1}</text>;
        })}

        {STRINGS.map((s, i) => {
          const y = padT + (STRINGS.length - 1 - i) * stringGap;
          // Ukulele strings are roughly equal-thickness nylon — keep visual variation subtle.
          const thickness = 1.2 + (STRINGS.length - 1 - i) * 0.15;
          return (
            <g key={i}>
        ...
        {positions.map((p, i) => {
          ...
          return (
            <g key={i}>
              <circle cx={x} cy={y} r={12} fill="#e11d48" stroke="#881337" strokeWidth={1.5}/>
              <text x={x} y={y + 4} fontSize={11} fontWeight="bold" textAnchor="middle" fill="white">
